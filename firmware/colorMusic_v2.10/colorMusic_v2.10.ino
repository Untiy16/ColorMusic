...
// settinngs visualization
bool isSettingsVisualizationActive = false;
...
void mainLoop() {
  ...
  if (ONstate) {
    if (millis() - main_timer > MAIN_LOOP) {
      ...
      if (!isSettingsVisualizationActive) {
        ...
        settingsVisualizationCountdown();
      }
      
      if (!IRLremote.receiving())    // если на ИК приёмник не приходит сигнал (без этого НЕ РАБОТАЕТ!)
        FastLED.show();         // отправить значения на ленту

      if (this_mode != 7 && !isSettingsVisualizationActive)       // 7 режиму не нужна очистка!!!
